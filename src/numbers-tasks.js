...
function getAverage(value1, value2) {
  ...
  return Math.min(average, Number.MAX_VALUE);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multiplicationOfVectors = x1 * x2 + y1 * y2;

  const vectorLength1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorLength2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosineOfAngle =
    multiplicationOfVectors / (vectorLength1 * vectorLength2);
...
function getSumOfDigits(num) {
  const arrOfNum = Array.from(String(num), Number);
  const sum = arrOfNum.reduce((acc, item) => acc + item, 0);
  return sum;
}
